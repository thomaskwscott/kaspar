...
class PathJsonValueRowDeserializerTest extends AnyFlatSpec {

  val COLUMN_NAME = "col1"
  val COLUMN_TYPE = ColumnType.INTEGER
  val COLUMN_PATH = "$.col1"
  val CONFIGURATION =
    s"""
      |{
      |  "columns" : [
  ...
  "A PathJsonValueRowDeserializer" should "load fieldMappings from configuration" in {
    val deserializer = new PathJsonValueRowDeserializer()
  ...
  "A PathJsonValueRowDeserializer" should "serialize fieldMappings" in {
    ...
    val deserialized = serIn.readObject().asInstanceOf[PathJsonValueRowDeserializer]
    ...
    assert(deserialized.fieldMappings(0)._3 == COLUMN_PATH)
  }
