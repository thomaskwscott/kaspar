...
class JsonKeyValueRowDeserializerTest extends AnyFlatSpec {

  val COLUMN_NAME = "col1"
  val COLUMN_TYPE = ColumnType.INTEGER
  val COLUMN_IS_KEY = false
  val CONFIGURATION =
    s"""
      |{
      |  "columns" : [
  ...
  "A JsonKeyValueRowDeserializer" should "load fieldMappings from configuration" in {
    val deserializer = new JsonKeyValueRowDeserializer()
  ...
  "A JsonKeyValueRowDeserializer" should "serialize fieldMappings" in {
    ...
    val deserialized = serIn.readObject().asInstanceOf[JsonKeyValueRowDeserializer]
    ...
    assert(deserialized.fieldMappings(0)._3 == COLUMN_IS_KEY)
  }
