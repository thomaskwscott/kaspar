...
class JsonValueRowDeserializer() extends RowDeserializer with Serializable {

  val fieldMappings : ListBuffer[(String,ColumnType)] = ListBuffer()
  ...
  override def configure(configuration: String) = {
    ...
    for( columnIndex <- 0 to columnCount-1)  {
      ...
      fieldMappings += ((columnName, ColumnType.withName(columnType)))
    }
  }
